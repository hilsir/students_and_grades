...
class MainController {
    async statistic(req, res, next) {
        ...
        const minGrade = async (subject) => {
            //console.log(await Assessment.min("grade",{where: {subject}}))
        ...
        const avgGrade = async (subject) => {
            const avg = (await Assessment.sum("grade", {
                    where: {
                        [Op.and]: [
                            {subject: subject},
                            {studentId: studentBd.id}
        ...
        const statistic = await Promise.all(subjectsArray.map(async (subject) => (({
    ...
    async log(req, res, next) {
        //const pagination = req.params.personalCode
        ...
        // Assessment.slice(startIndex, endIndex)
        // console.log()
